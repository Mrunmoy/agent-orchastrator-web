...
SCHEMA_PATH = Path(__file__).resolve().parents[1] / "src" / "agent_orchestrator" / "storage" / "schema.sql"
...
EXPECTED_COLUMNS = {
    "agent": [
        "id", "display_name", "provider", "model", "personality_key",
        "role", "status", "session_id", "capabilities_json",
        "created_at", "updated_at",
    ],
    "conversation": [
        "id", "title", "project_path", "state", "phase", "gate_status",
        "priority", "active", "summary_snapshot", "latest_artifact_id",
    ...
    "task": [
        ...
        "owner_agent_id", "depends_on_json", "started_at", "finished_at",
        "result_summary", "evidence_json", "created_at", "updated_at",
    ],
    "artifact": [
        "id", "conversation_id", "batch_id", "type", "payload_json", "created_at",
    ],
    "message_event": [
        "id", "conversation_id", "event_id", "source_type", "source_id",
        "text", "event_type", "metadata_json", "created_at",
    ],
    "scheduler_run": [
        "id", "conversation_id", "status", "batch_size", "reason",
        "started_at", "ended_at", "created_at",
    ],
    "resource_snapshot": [
        "id", "captured_at", "cpu_load_1m", "ram_used_mb", "ram_total_mb",
        "gpu_json", "agent_capacity_available",
    ],
}
...
def test_conversation_agent_references(db):
    ...
    db.execute(
        ...
        ("c1", "Test Conv", "/tmp", "debate", "design_debate", "open", 100, 1, None, None, now, now, None),
    )
    db.execute(
...
def test_message_event_autoincrement(db):
    ...
    db.execute(
        "INSERT INTO conversation VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
        ("c1", "Test", "/tmp", "debate", "design_debate", "open", 100, 0, None, None, now, now, None),
    )
    db.execute(
        "INSERT INTO message_event (conversation_id, event_id, source_type, source_id, text, event_type, created_at) "
        "VALUES (?,?,?,?,?,?,?)",
